# Kelle proxy (SpoofDPI 1.2.1) - PowerShell ile derleme ve kopyalama
...
$OutExe = Join-Path $Root "spoofdpi\kelle-proxy.exe"
$BinariesDir = Join-Path $Root "src-tauri\binaries"

# Go var mı?
$go = Get-Command go -ErrorAction SilentlyContinue
if (-not $go) {
    Write-Host "HATA: Go bulunamad?. PATH'e ekleyin veya https://go.dev/dl adresinden yukleyin." -ForegroundColor Red
    exit 1
}

# Kaynak klasoru var mi?
if (-not (Test-Path (Join-Path $SpoofDpiSrc "go.mod"))) {
    Write-Host "HATA: SpoofDPI kaynagi bulunamad?: $SpoofDpiSrc" -ForegroundColor Red
    exit 1
}
...
Write-Host "SpoofDPI (kelle-proxy) derleniyor..." -ForegroundColor Cyan
...
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "kelle-proxy.exe") -Force
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "kelle-proxy-x86_64-pc-windows-msvc.exe") -Force
Write-Host "Kopyaland?: src-tauri\binaries\kelle-proxy.exe (ve -x86_64-pc-windows-msvc.exe)" -ForegroundColor Green
